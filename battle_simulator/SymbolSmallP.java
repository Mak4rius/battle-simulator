...
public class SymbolSmallP extends Symbol implements SmallCase, Passive{
        ...
        if (this.numberIterationsAlive == 20) {
            for (int i = 0; i < Simulator.all_symbols.size(); i++) {
                ...
                if (Simulator.all_symbols.get(i).idSymbol == this.idSymbol) {
                    ...
                    SymbolCapitalP newborn = new SymbolCapitalP();
    ...
    public void escape() {
        ...
        for (int i = 0; i < Simulator.all_symbols.size(); i++) {
            ...
            if (current_symbol instanceof SymbolCapitalS){
    ...
    public void moveBreed() {
         ...
         */
        SymbolSmallP potential_parent = new SymbolSmallP();
        ...
        for (int i = 0; i < Simulator.all_symbols.size(); i++) {
            if(this.position.hashCode() == Simulator.all_symbols.get(i).position.hashCode()){
                SymbolSmallP newborn = new SymbolSmallP();
                ...
                if(((this.position.manhattanDistance(((SymbolSmallP) Simulator.passive_symbols.get(i)).position)) < this.sightDistance)){
                    if(this.position.manhattanDistance(potential_parent.getPosition()) > this.position.manhattanDistance(((SymbolSmallP) Simulator.passive_symbols.get(i)).getPosition())){
                        potential_parent = (SymbolSmallP) Simulator.passive_symbols.get(i);
                    }
                }
            }
        }
