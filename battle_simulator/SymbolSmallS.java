...
public class SymbolSmallS extends Symbol implements SmallCase, Passive {
        ...
        if (this.numberIterationsAlive == 20) {
            for (int i = 0; i < Simulator.all_symbols.size(); i++) {
                ...
                if (Simulator.all_symbols.get(i).idSymbol == this.idSymbol) {
                    ...
                    SymbolCapitalS newborn = new SymbolCapitalS();
    ...
    public void escape() {
        ...
        for (int i = 0; i < Simulator.all_symbols.size(); i++) {
            ...
            if (current_symbol instanceof SymbolCapitalR) {
                ...
                if ((i <= 10 & i > 0) & (j <= 10 & j > 0)) {
                    ...
                    if (closest_enemy.getPosition() != null) {
                        if (future_position.manhattanDistance(closest_enemy.getPosition()) > closest_distance) {
                            this.setPosition(future_position);
                            closest_distance = future_position.manhattanDistance(closest_enemy.getPosition());
                        }
                    }
                }
            }
    ...
    public void moveBreed(){
         ...
         */
        SymbolSmallS potential_parent = new SymbolSmallS();
        potential_parent.position.row = 100;
        potential_parent.position.column = 100;
        ...
        for (int i = 0; i < Simulator.all_symbols.size(); i++) {
            if(this.position.hashCode() == Simulator.all_symbols.get(i).position.hashCode()){
                SymbolSmallS newborn = new SymbolSmallS();
            ...
            else if(Simulator.passive_symbols.get(i).equals(this)){
                if(((this.position.manhattanDistance(((SymbolSmallS) Simulator.passive_symbols.get(i)).position)) < this.sightDistance)){
                    if(this.position.manhattanDistance(potential_parent.getPosition()) > this.position.manhattanDistance(((SymbolSmallS) Simulator.passive_symbols.get(i)).getPosition())){
                        potential_parent = (SymbolSmallS) Simulator.passive_symbols.get(i);
                    }
                }
            }
        }
